...
public class PreparedStatementBasic {
    ...
    public static void main(String[] args) {
        try {
            ...
            m = new Member(0, "test6", "1234", "이길동", "M", 26, "lee@test.com", "01012345678", "서울시 강남구", "야구", (Date) c.getTime(), null);
...
//			conn.commit(); // 수동으로 commit 명령을 내리는 방법
    ...
    public static List<Member> selectAll() throws SQLException {
        ...
        while (rs.next()) {
            ...
            Member m = new Member(mno, id, password, name, gender, age, email, phone, address, hobby, (Date) birthday, (Date) enrolldate);
            list.add(m);
        }
        pstmt.close();
        rs.close();
...
//        rs = pstmt.executeQuery(sql); // 쿼리 넣으면 리셋됨!!
        ...
        if (rs.next()) {
            ...
            m = new Member(no, id, pwd, name, gender, age, email, phone, address, hobby, (Date) birthday, (Date) enrolldate);
        }
        pstmt.close();
        rs.close();
        return m;
